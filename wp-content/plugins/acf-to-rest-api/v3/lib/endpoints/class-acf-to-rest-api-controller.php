...
if ( ! class_exists( 'ACF_To_REST_API_Controller' ) ) {
	class ACF_To_REST_API_Controller extends WP_REST_Controller {
				...
				foreach ( $data as $v ) {
					if ( isset( $v['acf'] ) ) {
						$response[] = array(
							...
							'created' => $v['date'],
							// 'test' => $v,
							'short' => $this->get_short_description($v['acf']['description']),
							'acf' => $v['acf'],
						);
					}
				}
		...
		public function get_short_description($description) {
		   $description_without_tags = strip_tags($description);
		   $end_position = strpos ($description_without_tags, ' ', 120);
		   $short_description = substr($description_without_tags,0,$end_position).'...';
